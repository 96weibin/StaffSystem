window.onload = function () {
	...
	**/
	...
	function login(event) {
		if(!loginUsernameInput.value || !loginPasswordInput.value){
			...
			myAjax.post('http://47.95.8.174/staff/asp/adminLogin.asp',obj,function(err, res){
				if(err){
					console.log(err)
				} else {
	...
	**/
	function sendStaffInfo () {
		if( dataTest()){
			...
			myAjax.post("http://47.95.8.174/staff/asp/addBaseInfo.asp",sendJsonBase,function (err, res) {
				if(err){
					console.log(err)
				}else{
					...
					if(json1.code === 1) {
						...
						for (var i = 0; i < sessionStorage.experienceCount; i++) {
							...
							myAjax.post("http://47.95.8.174/staff/asp/addExperienceInfo.asp",sendJsonExperience,function (err, res) {
								if(err) {
									console.log(err);
								} else{
									console.log(res)
	...
	**/
	function viewInfomation(){
		...
		myAjax.post("http://47.95.8.174/staff/asp/search.asp",{"Status":1},function (err,res) {
			if (err) {
				console.log(err);
			} else {
				// console.log(res)
	...
	**/
	function upData() {
		if (dataTest()) {
			myAjax.post("http://47.95.8.174/staff/asp/delete.asp",{Number: sendJsonBase.Number},function(err, res){
				if(err) {
					console.log(err)
				}else{
					sendStaffInfo();
	...
	function search(event) {
		if(!searchInput.value) {
			...
			if(isNaN(searchInput.value)){
				// console.log("不能转为数字")
				myAjax.post("http://47.95.8.174/staff/asp/search.asp",{"Status":"3","Value":searchInput.value}, function(err,res){
					if(err){
						console.log(err);
					}else{
						moreInfoJson = JSON.parse(res);
				...
				// console.log("能")
				myAjax.post("http://47.95.8.174/staff/asp/search.asp",{"Status":"2","Value":searchInput.value}, function(err,res){
					if(err){
						console.log(err);
					}else{
						moreInfoJson = JSON.parse(res);
		...
		if(sessionStorage.logInStatus !== "管理员登录"){
			...
			if(event.target.dataset.flag === "0"){
				...
				for (var i = 0; i < detailInfoBox.length; i ++) {
					...
					detailInfoBox[i].previousSibling.children[5].style.backgroundImage = "url(http://47.95.8.174/staff/img/pen.png)";
				...
				myAjax.post("http://47.95.8.174/staff/asp/search.asp",{"Status": 4, "value":event.target.parentNode.children[0].children[0].value},function(err,res) {
					if(err) {
						console.log(err);
					}else{
						// console.log(res);
	...
	function deleteViewInfoItem(event) {
		if(sessionStorage.logInStatus !== "管理员登录"){
			...
			if(window.confirm(`确定要删除${num}此条信息？`)){
				myAjax.post("http://47.95.8.174/staff/asp/delete.asp",{Number : this.parentNode.children[0].children[0].value},function (err, res) {
					if(err) {
						console.log(err);
					} else {
						alert(num + "的信息删除成功");
		...
		if(sessionStorage.logInStatus !== "管理员登录"){
			...
			if(event.target.dataset.flag === "0"){
				...
				for (var i = 0; i < oDetailInfoBox.length; i++) {
					...
					oDetailInfoBox[i].previousSibling.children[5].style.backgroundImage = "url(http://47.95.8.174/staff/img/pen.png)";
				...
				this.style.backgroundImage="url(http://47.95.8.174/staff/img/right.png)"
				...
				if (msg !== "是否要将") {
					...
					if(confirm(msg)){
						myAjax.post("http://47.95.8.174/staff/asp/delete.asp",{"Number":oNumber},function(err, res) {
							if(err){
								console.log(err)
							}else {
								console.log(sendJsonBase);
								...
								if(JSON.parse(res).code === 0) {
									myAjax.post("http://47.95.8.174/staff/asp/addBaseInfo.asp",sendJsonBase,function (err, res) {
										if(err){
											...
											for (var i = 0, count = sendJsonExperience.count; i < count; i ++) {
												myAjax.post("http://47.95.8.174/staff/asp/addExperienceInfo.asp",sendJsonExperience[i],function (err, res) {
													if(err){
														console.log(err);
														return
													}else {}
									...
									event.target.setAttribute("data-flag",0);
									// event.target.style.transform = "rotate(0deg)";
									event.target.style.backgroundImage = "url(http://47.95.8.174/staff/img/pen.png)"
